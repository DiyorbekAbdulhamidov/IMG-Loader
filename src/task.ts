import { User, Repository, Branch, Commit } from "./types";
...
function getRepositories(username: string): Promise<Repository[]> {
  ...
  return new Promise((solve) =>
    setTimeout(() => solve(repositories.filter((r) => r.ownerUsername === username)), time)
  );
}
...
function getBranches(repoID: string): Promise<Branch[]> {
  ...
  return new Promise((solve) =>
    setTimeout(() => solve(branches.filter((b) => b.repoId === repoID)), time)
  );
}
...
function getCommits(branchID: string): Promise<Commit[]> {
  ...
  return new Promise((solve) =>
    setTimeout(() => solve(commits.filter((c) => c.branchId === branchID)), time)
  );
}
